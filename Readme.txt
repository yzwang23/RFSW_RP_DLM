...
Accompanying this software package are four examples of XML exports of two OCT volume scans, obtained using Spectralis SD-OCT from the left eye of a patient on 2 separate visits in the folder "Volume_Scan_Examples/XML_RP_Auto_HE_Manu_YZW_ID_P22". These XML exports are included for demonstrating the use of the software package. Each volume scan has two XLM exports: one with the layer boundary segmentation results by the Spectralis' built-in automatic segmentation software, and the other with manual correction or segmentation upon the automatic segmentation by Spectralis. The following are the instructions on how to run this software package.
