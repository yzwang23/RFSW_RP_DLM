...
% which DL model's results to display
kD = 1;                   % default -> first DLM
...
if strcmpi(SegType, 'Auto') && isfield(Segments, 'Auto')
  Segment = Segments.Auto(1).Segment;
elseif strcmpi(SegType, 'Manu') && isfield(Segments, 'Manual')
  Segment = Segments.Manual(1).Segment;
elseif strcmpi(SegType, 'DMLM') && isfield(Segments, 'ML')
  Segment = Segments.ML(1).SegmentData.Segment;
elseif strcmpi(SegType, 'HDMLM') && isfield(Segments, 'ML')
  Segment = Segments.ML(1).SegmentData_Hybrid.Segment;
else
  disp('no segmentation type defined. return ...');
end

% for DMLM & HDMLM, check to remove isolated EZ segments or not
if strcmpi(SegType, 'DMLM') && elimIsoEZSegment~=0
  OCTScan = UNetPPDeleteIsolatedLayerSegment(OCTScan, LN1, LN2, 'DMLM', kD);
end

% get 'HDMLM' hybrid deep machine learning model segmentation results
if strcmpi(SegType, 'HDMLM') && elimIsoEZSegment~=0
